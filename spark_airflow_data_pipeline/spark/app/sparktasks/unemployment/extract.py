...
class Extract:
    ...
    def __init__(self):
        ...
        self.spark = SparkSession.builder.appName('ExtractCovid').getOrCreate()
        self.config = Config()
    ...
    def filter_df(self, df_to_filter, key):
        ...
        if row:
            record_date = row[4]
            if record_date.month == 12:
                return df_to_filter.filter((df_to_filter.Results_series_data_period >='M01') & (df_to_filter.Results_series_data_year>record_date.year))
            if record_date.month<10:
                month_name = 'M0'+str(record_date.month)
            else:
                month_name = 'M'+str(record_date.month)
            return df_to_filter.filter((df_to_filter.Results_series_data_period>month_name) & (df_to_filter.Results_series_data_year>=record_date.year))
        return df_to_filter
